...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mpCloudImageSampler = new CloudImageSampler(this, nSamplerEdgeFrontKFNum, nSamplerEdgeBackKFNum, nSamplerEdgeFrontMinTime, nSamplerEdgeBackMinTime, samplerPDKp, samplerPDKd, samplerPDth);
    mpTracker->SetCloudImageSampler(mpCloudImageSampler);
    ...
    mpLocalMapper->SetOpenKFCulling(bKFCulling);
    if (settings_)
    ...
    mpCloudMerger = new CloudMerging(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor != MONOCULAR, activeLC, bCloudMerge, bMergeAnyway, mpMapDrawer, mpFrameDrawer);
    mptCloudMerging = new thread(&ORB_SLAM3::CloudMerging::Run, mpCloudMerger, bCloudOnline);

    //Set pointers between threads
    mpTracker->SetLocalMapper(mpLocalMapper);
    mpTracker->SetLoopClosing(mpLoopCloser);
    mpTracker->SetCloudMerging(mpCloudMerger);

    mpLocalMapper->SetTracker(mpTracker);
    mpLocalMapper->SetLoopCloser(mpLoopCloser);
    mpLocalMapper->SetCloudMerging(mpCloudMerger);

    mpLoopCloser->SetTracker(mpTracker);
    mpLoopCloser->SetLocalMapper(mpLocalMapper);
    mpLoopCloser->SetCloudMerging(mpCloudMerger);

    mpCloudMerger->SetTracker(mpTracker);
    mpCloudMerger->SetLocalMapper(mpLocalMapper);
    mpCloudMerger->SetLoopClosing(mpLoopCloser);

    mpMapDrawer->SetCloudMerging(mpCloudMerger);
...
void System::PublishCloudImages(std::vector<CloudImage> vCloudImages, std::vector<CloudImage> vCloudNoSamplingImages, int edgeFrontMapId, int edgeBackMapId) {
    // @note print
    cout << "System: Receive Cloud Images !" << endl;
    mvCurrentCloudProcessImages = vCloudImages;
    mvCurrentCloudProcessNoSamplingImages = vCloudNoSamplingImages;
    mEdgeFrontMapId = edgeFrontMapId;
    mEdgeBackMapId = edgeBackMapId;
}
...
void System::GetCloudProcessImages(std::vector<CloudImage> &vCloudProcessImages, std::vector<CloudImage> &vCloudProcessNoSamplingImages, int &edgeFrontMapId, int &edgeBackMapId) {
    vCloudProcessImages = mvCurrentCloudProcessImages;
    vCloudProcessNoSamplingImages = mvCurrentCloudProcessNoSamplingImages;
    edgeFrontMapId = mEdgeFrontMapId;
    edgeBackMapId = mEdgeBackMapId;
}

void System::ResetCloudProcessImages() {
    mvCurrentCloudProcessImages.clear();
}

void System::SetTrackLostTimestamp(double t) {
    mpTracker->SetNewTimeStampLost(t);
}
...
void System::SaveAtlasCustom(Atlas *atlas, const string &savePath, int type) {
    //clock_t start = clock();
    ...
    //        pathSaveFileName = pathSaveFileName.append(mStrSaveAtlasToFile);
    //        pathSaveFileName = pathSaveFileName.append(".osa");
    const string &pathSaveFileName = savePath;
...
bool System::LoadAtlasCustom(Atlas *atlas, const string &loadPath, int type) {
    string strFileVoc, strVocChecksum;
    bool isRead = false;
    ...
    //    pathLoadFileName = pathLoadFileName.append(mStrLoadAtlasFromFile);
    //    pathLoadFileName = pathLoadFileName.append(".osa");
    const string &pathLoadFileName = loadPath;
...
ORBextractor *System::GetInitExtractor() {
    return mpTracker->GetInitExtractor();
}

ORBextractor *System::GetExtractor() {
    return mpTracker->GetExtractor();
}

ORBVocabulary *System::GetVocabulary() {
    return mpTracker->GetVocabulary();
}

GeometricCamera *System::GetCamera() {
    return mpTracker->GetCamera();
}

cv::Mat System::GetDistCoef() {
    return mpTracker->GetDistCoef();
}

float System::Getbf() {
    return mpTracker->Getbf();
}

float System::GetThDepth() {
    return mpTracker->GetThDepth();
}

IMU::Calib *System::GetImcCalib() {
    return mpTracker->GetImcCalib();
}
