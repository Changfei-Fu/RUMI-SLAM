...
void LoopClosing::SetCloudMerging(CloudMerging *pCloudMerging) {
    ...
    while (1) {
        ...
        if (CheckNewKeyFrames()) {
            ...
            if (bFindedRegion) {
                ...
                if (mbMergeDetected) {
                    if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD) && (!mpCurrentKF->GetMap()->isImuInitialized())) // IMU的，这部分if不用管
                    {
                        cout << "IMU is not initilized, merge is aborted" << endl;
                    } else {
                        Sophus::SE3d mTmw = mpMergeMatchedKF->GetPose().cast<double>(); // 从Place Recognition成功的结果中取出匹配的关键帧
                        ...
                        if (mpCurrentKF->GetMap()->IsInertial() && mpMergeMatchedKF->GetMap()->IsInertial()) // IMU的，这部分if不用管
                        {
                            ...
                            if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD) && mpCurrentKF->GetMap()->GetIniertialBA1()) {
                    ...
                    if (mbLoopDetected) {
                        // ***************** 如果同时存在Map Merge和Loop Closure，仅进行Map Merge，不进行Loop Closure，我觉得是因为地图已发生了改变，按理说这种情况基本不会发生 *****************
                ...
                if (mbLoopDetected) {
                    ...
                    if (mpCurrentKF->GetMap()->IsInertial()) // IMU的，这部分if不用管
                    {
                        ...
                        if (fabs(phi(0)) < 0.008f && fabs(phi(1)) < 0.008f && fabs(phi(2)) < 0.349f) {
                            if (mpCurrentKF->GetMap()->IsInertial()) {
                                // If inertial, force only yaw
                                if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD) && mpCurrentKF->GetMap()->GetIniertialBA2()) {
                                    phi(0) = 0;
                                    phi(1) = 0;
...
bool LoopClosing::NewDetectCommonRegions() {
...
#endif
    // ***************** 第三步（Loop Closure），当上一个关键帧有匹配Place Recognition成功的迹象，这一帧进行确认检查 *****************
    if (mnLoopNumCoincidences > 0) {
        ...
        if (bCommonRegion) {
            ...
            mnLoopNumCoincidences++; // 若新一关键帧也匹配成功，则计数+1
            mpLoopLastCurrentKF->SetErase();
            mpLoopLastCurrentKF = mpCurrentKF;
            mg2oLoopSlw = gScw;
            mvpLoopMatchedMPs = vpMatchedMPs;

            mbLoopDetected = mnLoopNumCoincidences >= 3; // 只要mnLoopNumCoincidences大于3，直接判定Place Recognition成功
            mnLoopNumNotFound = 0;
...
void LoopClosing::MergeLocal() // 无IMU情况下的子图合并
{
    ...
    // ***************** 第五步，迁移关键帧和Map Point，根据Active Map中关键帧和Map Point计算好的Merge Map坐标系下的Pose、坐标，进行迁移 *****************
    {
        ...
        pMergeMap->mvMergedMapIds.push_back(pCurrentMap->GetId());
    }
...
void LoopClosing::MergeLocal2() // IMU情况下的子地图合并
{
