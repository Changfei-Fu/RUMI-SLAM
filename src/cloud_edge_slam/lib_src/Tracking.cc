...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase *pKFDB, const string &strSettingPath, const int sensor, Settings *settings, const string &_nameSeq) :
    mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false), mbSetImgGray(false),
...
void Tracking::SetCloudMerging(CloudMerging *pCloudMerging) {
...
void Tracking::SetCloudImageSampler(CloudImageSampler *pCloudImageSampler) {
    mpCloudImageSampler = pCloudImageSampler;
}
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename) {
    ...
    mCurrentFrame.SetImgGray(mImBGR);
...
void Tracking::Track() {
    ...
    Map *pLastMap = mpAtlas->GetLastMap();
    if (!pCurrentMap) {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        // @note CloudImageSampler Step
        mpCloudImageSampler->TrackStep(mState, false, pCurrentMap->KeyFramesInMap(), mpAtlas->GetCurrentMap()->GetId(), mImBGR, mCurrentFrame.mTimeStamp);
            ...
            if (mState == OK) {
                ...
                if (!bOK) {
                    if (mCurrentFrame.mnId <= (mnLastRelocFrameId + mnFramesToResetIMU) && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)) {
                        mState = LOST;
                    } else if (pCurrentMap->KeyFramesInMap() > mpCloudImageSampler->nNewTrackFirst) {
                ...
                if (mState == RECENTLY_LOST) {
                    ...
                    // @note Main LOST
                    // 感觉很奇怪，按理说不会进这里，如果LOST，在下面的那个LOST的地方就会直接到NOT_IMAGE_YET了
                    Verbose::PrintMess("A new map is started...", Verbose::VERBOSITY_NORMAL);

                    if (pCurrentMap->KeyFramesInMap() <= mpCloudImageSampler->nNewTrackFirst || pCurrentMap->KeyFramesDuration() <= mpCloudImageSampler->nNewTrackFirstMinTime || (pCurrentMap->ComputeTrajCurvature() <= mpCloudImageSampler->nNewTrackFirstMinTrajCurvature)) {
                        mpSystem->ResetActiveMap();
        ...
        if (bOK)
            ...
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) {
                ...
                // @note Track_Local_Map RECENTLY_LOST
        ...
        if (bOK || mState == RECENTLY_LOST) {
            // @note Track_Local_Map OK or RECENTLY_LOST
            // Update motion model
            ...
            // bool bNeedKF = CloudNeedNewKeyFrame(); // TODO Use Cloud Version
        ...
        // @note CloudImageSampler Step
        mpCloudImageSampler->TrackStep(mState, bIsKF, pCurrentMap->KeyFramesInMap(), mpAtlas->GetCurrentMap()->GetId(), mImBGR, mCurrentFrame.mTimeStamp);

        // Reset if the camera get lost soon after initialization
        if (mState == LOST) {
            if (pCurrentMap->KeyFramesInMap() <= mpCloudImageSampler->nNewTrackFirst || pCurrentMap->KeyFramesDuration() <= mpCloudImageSampler->nNewTrackFirstMinTime || (pCurrentMap->ComputeTrajCurvature() <= mpCloudImageSampler->nNewTrackFirstMinTrajCurvature)) {
                cerr << "######################################" << endl;
                cerr << "Reset Current Map: " << endl;
                cerr << "KF Num: " << pCurrentMap->KeyFramesInMap() << "   /   " << mpCloudImageSampler->nNewTrackFirst << endl;
                cerr << "Duration: " << pCurrentMap->KeyFramesDuration() << "   /   " << mpCloudImageSampler->nNewTrackFirstMinTime << endl;
                cerr << "Curvature: " << pCurrentMap->ComputeTrajCurvature() << "   /   " << mpCloudImageSampler->nNewTrackFirstMinTrajCurvature << endl;
                cerr << "######################################" << endl;
                mpSystem->ResetActiveMap();
                return;
            }
...
bool Tracking::CloudNeedNewKeyFrame() {
    // 添加使得CloudMap Keyframe被选中的约束
    bool bTestKeyFrame = false; // additional condition
    double curTimeStamp = mCurrentFrame.mTimeStamp;
    auto iter_begin = mpCloudMerging->mvpTestCloudMapTimeStamps.begin();
    auto iter_end = mpCloudMerging->mvpTestCloudMapTimeStamps.end();
    if (std::find(iter_begin, iter_end, curTimeStamp) != iter_end) {
        bTestKeyFrame = true;
        cout << "Test Need Cur Frame" << endl;
    } else {
    }
    //     std::vector<double> deltaTimes(mpCloudMerging->mvpTestCloudMapTimeStamps.size());
    //     for (int i = 0; i < mpCloudMerging->mvpTestCloudMapTimeStamps.size(); ++i) {
    //         deltaTimes[i] = abs(mpCloudMerging->mvpTestCloudMapTimeStamps[i] - curTimeStamp);
    //     }
    //     if (*min_element(deltaTimes.begin(), deltaTimes.end()) < 0.001) {
    //         bTestKeyFrame = true;
    // //        cout << "Test Need Cur Frame" << endl;
    //     }
    ...
    // Do not insert keyframes if not enough frames have passed from last relocalisation
    if (!bTestKeyFrame && (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames && nKFs > mMaxFrames)) {
        return false;
    }
    ...
    if (((bTestKeyFrame || c1a || c1b || c1c) && c2) || c3 || c4) {
        // If the mapping accepts keyframes, insert keyframe.
        // Otherwise send a signal to interrupt BA
        if (bLocalMappingIdle || mpLocalMapper->IsInitializing()) {
            return true;
...
void Tracking::ResetActiveMap(bool bLocMap) {
    ...
    // @note
    mpCloudImageSampler->UpdateResetMapImages(pMap);
...
void Tracking::SetNewTimeStampLost(double t) {
    mTimeStampLost = t;
}
...
ORBextractor *Tracking::GetInitExtractor() {
    return mpIniORBextractor;
}

ORBextractor *Tracking::GetExtractor() {
    return mpORBextractorLeft;
}

ORBVocabulary *Tracking::GetVocabulary() {
...
IMU::Calib *Tracking::GetImcCalib() {
    return mpImuCalib;
}
