...
void Atlas::ClearOldMapImages() {
    auto vpMaps = vector<Map *>(mspMaps.begin(), mspMaps.end());
    if (vpMaps.size() > 2) {
        for (auto iterpMap = vpMaps.begin(); iterpMap != (vpMaps.end() - 2); iterpMap++) {
            (*iterpMap)->ClearKeyframeImages();
        }
    }
}
...
void Atlas::InsertNewMap(Map *pMap) {
    unique_lock<mutex> lock(mMutexAtlas);
    cout << "Insertion of new map with id: " << pMap->GetId() << endl;
    if (mpCurrentMap) {
        if (!mspMaps.empty() && mnLastInitKFidMap < mpCurrentMap->GetMaxKFid())
            mnLastInitKFidMap = mpCurrentMap->GetMaxKFid() + 1; //The init KF is the next of current maximum
...
Map *Atlas::GetCurrentMap() {
    ...
    while (mpCurrentMap->IsBad()) {
        usleep(3000);
        cerr << "Atlas: current map is bad" << endl;
    }

    return mpCurrentMap;
}

Map *Atlas::GetLastMap() {
    vector<Map *> vpMaps = GetAllMaps();
    if (vpMaps.size() >= 2) {
        return vpMaps[vpMaps.size() - 2];
    } else {
        return NULL;
    }
}

Map *Atlas::GetSpecifyMap(int mnId) {
    Map *resultMap = nullptr;
    // 精准匹配
    for (Map *pMap_i : mspMaps) {
        int p_mnId = pMap_i->GetId();
        if (mnId == p_mnId) {
            resultMap = pMap_i;
            return resultMap;
        }
    ...
    // 在Merge中匹配
    for (Map *pMap_i : mspMaps) {
        if (count(pMap_i->mvMergedMapIds.begin(), pMap_i->mvMergedMapIds.end(), mnId)) {
            resultMap = pMap_i;
            return resultMap;
        }
    }
...
Map *Atlas::GetLargestMap() {
    unique_lock<mutex> lock(mMutexAtlas);
    struct compFunctor {
        inline bool operator()(Map *elem1, Map *elem2) {
            return elem1->KeyFramesInMap() < elem2->KeyFramesInMap();
        }
    };
    vector<Map *> vMaps(mspMaps.begin(), mspMaps.end());
    sort(vMaps.begin(), vMaps.end(), compFunctor());
    return vMaps[vMaps.size() - 1];
}

Map *Atlas::GetDistanceLongestMap() {
    unique_lock<mutex> lock(mMutexAtlas);
    struct compFunctor {
        inline bool operator()(Map *elem1, Map *elem2) {
            return elem1->KeyFramesLength() < elem2->KeyFramesLength();
        }
    };
    vector<Map *> vMaps(mspMaps.begin(), mspMaps.end());
    sort(vMaps.begin(), vMaps.end(), compFunctor());
...
Map *Atlas::GetDurationLongestMap() {
    unique_lock<mutex> lock(mMutexAtlas);
    struct compFunctor {
        inline bool operator()(Map *elem1, Map *elem2) {
            return elem1->KeyFramesDuration() < elem2->KeyFramesDuration();
        }
    };
    vector<Map *> vMaps(mspMaps.begin(), mspMaps.end());
    sort(vMaps.begin(), vMaps.end(), compFunctor());
