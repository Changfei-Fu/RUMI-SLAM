...
public:
    Point(const float &acc_x, const float &acc_y, const float &acc_z,
        ...
        a(acc_x, acc_y, acc_z),
        w(ang_vel_x, ang_vel_y, ang_vel_z), t(timestamp) {
    }
...
public:
    ...
    Bias(const float &b_acc_x, const float &b_acc_y, const float &b_acc_z,
        ...
        bax(b_acc_x),
        bay(b_acc_y), baz(b_acc_z), bwx(b_ang_vel_x), bwy(b_ang_vel_y), bwz(b_ang_vel_z) {
    }
...
public:
    ...
    void printMeasurements() const {
        std::cout << "pint meas:\n";
        for (int i = 0; i < static_cast<int>(mvMeasurements.size()); i++) {
