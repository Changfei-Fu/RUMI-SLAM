...
void SaveKeyFrameTrajectoryTUM(Map *map, const std::string &filename);
void SaveSelectedKeyFrameTrajectoryTUM(Map *map, std::vector<int> &vSelected, const std::string &filename);
void CloudExportKeyFrameTrajectoryTUM(Map *map, std::vector<double> &vTimestamp, vector<Eigen::Vector3f> &vPosition, vector<Eigen::Quaternionf> &vQuaternion);
void CloudSaveKeyFrameTrajectoryTUM(Map *map, const std::string &filename);
...
public:
    ...
    void UpdateCloudConnections(bool upParent = true); // 主要是处理Cloud Map生成时，KF互相为Parent的问题，其他暂时不完全保持时序一致性（即不完全符合本身按时序添加关键帧的逻辑）
    ...
    // Set Img
    void SetImgGray(const cv::Mat &imgGray);
    void ClearImgGray();
