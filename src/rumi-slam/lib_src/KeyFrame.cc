...
void SaveKeyFrameTrajectoryTUM(Map *map, const string &filename) {
    cout << endl
...
void SaveSelectedKeyFrameTrajectoryTUM(Map *map, std::vector<int> &vSelected, const std::string &filename) {
    cout << endl
         << "Saving keyframe trajectory to " << filename << " ..." << endl;

    vector<KeyFrame *> vpKFs = map->GetAllKeyFrames();
    vector<KeyFrame *> vpSelectedKFs;
    for (auto &i : vSelected) {
        vpSelectedKFs.push_back(vpKFs[i]);
    }
    sort(vpSelectedKFs.begin(), vpSelectedKFs.end(), KeyFrame::lId);
    ...
    for (size_t i = 0; i < vpSelectedKFs.size(); i++) {
        KeyFrame *pKF = vpSelectedKFs[i];
...
void CloudExportKeyFrameTrajectoryTUM(Map *map, vector<double> &vTimestamp, vector<Eigen::Vector3f> &vPosition, vector<Eigen::Quaternionf> &vQuaternion) {
    cout << "Export KeyFrame Trajectory" << endl;
    vTimestamp.clear();
    vPosition.clear();
    vQuaternion.clear();
    ...
    vector<KeyFrame *> vpKFs = map->GetAllKeyFrames();
    // sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);
    sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lTs);
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        bool bRepeat = false;
        for (auto &ts : vpTSs) {
            if (abs(pKF->mTimeStamp - ts) < 0.0001) {
                bRepeat = true;
            }
        }
        if (bRepeat && pKF->isCloud()) {
            continue;
        } else {
            vpTSs.push_back(pKF->mTimeStamp);
        }
        ...
        vTimestamp.push_back(pKF->mTimeStamp);
        vPosition.push_back(t);
        vQuaternion.push_back(q);
    }
}

void CloudSaveKeyFrameTrajectoryTUM(Map *map, const string &filename) {
    cout << endl
         << "Saving keyframe trajectory to " << filename << " ..." << endl;
...
KeyFrame::KeyFrame() :
    ...
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(-1),
    ...
    mbToBeErased(false), mbBad(false), mbCloud(false), mHalfBaseline(0), mbCurrentPlaceRecognition(false), mnMergeCorrectedForKF(0),
    NLeft(0), NRight(0), mnNumberOfOpt(0), mbHasVelocity(false) {
}

KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
    bImu(pMap->isImuInitialized()), mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), imgGray(F.imgGray), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
    ...
    mbToBeErased(false), mbBad(false), mbCloud(false), mHalfBaseline(F.mb / 2), mpMap(pMap), mbCurrentPlaceRecognition(false), mNameFile(F.mNameFile), mnMergeCorrectedForKF(0),
...
void KeyFrame::UpdateCloudConnections(bool upParent) {
    map<KeyFrame *, int> KFcounter;
    ...
    // ***************** 更新拓展树（父子），最佳匹配 *****************
    {
        ...
        // TODO my work: filter newer timestamps keyframe
        vector<KeyFrame *> vpFilterOrderedConnectedKeyFrames;
        for (auto pKeyFrame : mvpOrderedConnectedKeyFrames) {
            if (pKeyFrame->mTimeStamp < this->mTimeStamp) {
                vpFilterOrderedConnectedKeyFrames.push_back(pKeyFrame);
            }
        }

        if (mbFirstConnection && vpFilterOrderedConnectedKeyFrames.size() && mnId != mpMap->GetInitKFid()) {
            mpParent = vpFilterOrderedConnectedKeyFrames.front(); // 取共视最高的一帧作为父亲
            mpParent->AddChild(this);
            mbFirstConnection = false;
        }
    }
...
void KeyFrame::SetCloudFlag() {
    mbCloud = true;
}

bool KeyFrame::isCloud() {
    return mbCloud;
}

void KeyFrame::SetInitFlag() {
    mbInit = true;
}
...
void KeyFrame::SetImgGray(const cv::Mat &imgGray) {
    this->imgGray = imgGray.clone();
}

void KeyFrame::ClearImgGray() {
