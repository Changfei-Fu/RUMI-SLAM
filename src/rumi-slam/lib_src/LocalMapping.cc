...
void LocalMapping::SetCloudMerging(CloudMerging *pCloudMerging) {
...
void LocalMapping::CreateNewMapPoints() {
    ...
    for (size_t i = 0; i < vpNeighKFs.size(); i++) {
        ...
        for (int ikp = 0; ikp < nmatches; ikp++) {
            ...
            const cv::KeyPoint &kp1 = (mpCurrentKeyFrame->NLeft == -1) ? mpCurrentKeyFrame->mvKeysUn[idx1] : (idx1 < mpCurrentKeyFrame->NLeft) ? mpCurrentKeyFrame->mvKeys[idx1] : mpCurrentKeyFrame->mvKeysRight[idx1 - mpCurrentKeyFrame->NLeft];
...
void LocalMapping::Release() {
    ...
    // @note Cloud Merging完成时调用Release有可能会清空新Map的两个初始化帧导致Bug
...
void LocalMapping::SetOpenKFCulling(bool b) {
    mbKFCulling = b;
}
...
void LocalMapping::CloudKeyFrameCulling() {
    ...
    for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(), vend = vpLocalKeyFrames.end(); vit != vend; vit++) {
        ...
        // @note if cloud KF, continue, because no Bow vector
        if (pKF->isCloud()) {
            continue;
        }
            ...
            if (pMP) {
                if (!pMP->isBad()) {
                    ...
                    if (pMP->Observations() > thObs) {
                        const int &scaleLevel = (pKF->NLeft == -1) ? pKF->mvKeysUn[i].octave : (i < pKF->NLeft) ? pKF->mvKeys[i].octave : pKF->mvKeysRight[i].octave;
