...
Sim3Solver::Sim3Solver(KeyFrame *pKF1, KeyFrame *pKF2, const vector<MapPoint *> &vpMatched12, const bool bFixScale,
    ...
    mnIterations(0),
    mnBestInliers(0), mbFixScale(bFixScale),
    ...
    const vector<MapPoint *> &vpKeyFrameMP1 = pKF1->GetMapPointMatches();
...
Eigen::Matrix4f Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers, bool &bConverge,
    const std::vector<KeyFrame *> &Map1KFs, const std::vector<KeyFrame *> &Map2KFs, const std::vector<std::vector<std::pair<int, int>>> &avpValidKPMatches, 
    float &bestRatio, Eigen::Matrix3f &bestRotation,Eigen::Vector3f &bestTranslation, float &bestScale) {
    bNoMore = false;
    bConverge = false;
    vbInliers = vector<bool>(mN1, false);
    nInliers = 0;
    ...
    while (mnIterations < mRansacMaxIts && nCurrentIterations < nIterations) {
        ...
        // 利用所有匹配的MapPoint计算inlierRatio，而不仅使用当前KF对的MapPoint
        Eigen::Matrix3d solveR;
        Eigen::Vector3d solveT;
        double solveS;
        g2o::Sim3 gRANSACSw1w2;
        // get solve result
        solveR = mR12i.cast<double>();
        solveT = mt12i.cast<double>();
        solveS = (double)ms12i;
        g2o::Sim3 gSc1c2(solveR, solveT, solveS);
        g2o::Sim3 gSc1w(mpKF1->GetRotation().cast<double>(), mpKF1->GetTranslation().cast<double>(), 1.0); // g2o::Sim3: kf 1 world -> kf 1 camera
        g2o::Sim3 gSc2w(mpKF2->GetRotation().cast<double>(), mpKF2->GetTranslation().cast<double>(), 1.0); // g2o::Sim3: kf 2 world -> kf 2 camera
        gRANSACSw1w2 = gSc1w.inverse() * gSc1c2 * gSc2w;
        float InliersRatio = ComputeInliersNum(Map1KFs, Map2KFs, avpValidKPMatches, gRANSACSw1w2);

        if (InliersRatio >= bestRatio && mnInliersi >= mnBestInliers){
            mvbBestInliers = mvbInliersi;
            mnBestInliers = mnInliersi;

            bestRatio = InliersRatio;
            mBestT12 = mT12i;
            mBestRotation = mR12i;
            mBestTranslation = mt12i;
            mBestScale = ms12i;
            ...
            // 若InlierRatio大于阈值则判定为收敛，退出循环
            if (InliersRatio > 0.10 && mnInliersi > mRansacMinInliers) {
                bConverge = true;
                // cout << "converge inlier ratio: " << InliersRatio << endl;;
                return mBestT12;
            } else {
                bestSim3 = mBestT12;
            }
...
Eigen::Matrix4d Sim3Solver::umeyamaSolve(const vector<Eigen::Vector3d> &srcMatchPoints, const vector<Eigen::Vector3d> &dstMatchPoints) {
...
void Sim3Solver::ComputeSim3(Eigen::Matrix3f &P1, Eigen::Matrix3f &P2) {
    ...
    // @note first merge，nan错误，这个bug在官方issue中出现了很多次，很可能就是有bug
    // @note add this by https://github.com/UZ-SLAMLab/ORB_SLAM3/issues/608，没有用，我选择跳过当次求解
...
float Sim3Solver::ComputeInliersNum(const std::vector<KeyFrame *> &map1KFs, const std::vector<KeyFrame *> &map2KFs, const std::vector<std::vector<std::pair<int, int>>> &avpValidKPMatches, g2o::Sim3 &gSw1w2) {
