...
void Optimizer::OptimizeEssentialGraph(Map *pMap, KeyFrame *pLoopKF, KeyFrame *pCurKF,
    ...
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
        ...
        if (pMP->mnCorrectedByKF == pCurKF->mnId) {
            ...
            // Temp措施，不知道为什么会有这个Bug
            if (!pRefKF) {
                cerr << "My Loop Closing Error" << endl;
                continue;
            }
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2,
    ...
    vSim3->setId(0);
    vSim3->setFixed(false);
    vSim3->pCamera1 = pKF1->mpCamera; // 赋值了camera_1的成员变量
    vSim3->pCamera2 = pKF2->mpCamera; // 赋值了camera_2的成员变量
    optimizer.addVertex(vSim3);
    ...
    for (int i = 0; i < N; i++) {
        ...
        nCorrespondences++; // 正常的MapPoint匹配对数量
...
float Optimizer::OptimizeCloudSim3(const std::vector<KeyFrame *> &map1KFs, const std::vector<KeyFrame *> &map2KFs, const std::vector<std::vector<MapPoint *>> &avpMatches,
                                   g2o::Sim3 &gSw1w2, const float th2, const bool bFixScale, Eigen::Matrix<double, 7, 7> &mAcumHessian, const bool bAllPoints) {
    g2o::SparseOptimizer optimizer;
    ...
    g2o::OptimizationAlgorithmLevenberg *solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    // g2o::OptimizationAlgorithmGaussNewton *solver = new g2o::OptimizationAlgorithmGaussNewton(solver_ptr);  // GaussNetwon求解器没有区别
    optimizer.setAlgorithm(solver);

    // Set Sim3 vertex
    ORB_SLAM3::VertexSim3Expmap *vSim3 = new ORB_SLAM3::VertexSim3Expmap();
    vSim3->_fix_scale = bFixScale; // false
    vSim3->setEstimate(gSw1w2);
    vSim3->setId(0);
    vSim3->setFixed(false);
    vSim3->pCamera1 = map1KFs[0]->mpCamera; // 赋值了camera_1的成员变量  // TODO CloudMap构建时注意要添加上这个变量
    vSim3->pCamera2 = map2KFs[0]->mpCamera; // 赋值了camera_2的成员变量   // TODO CloudMap构建时注意要添加上这个变量
    // @note second merge addVertex报错，不一定是这一行
    optimizer.addVertex(vSim3);
    ...
    // result variables
    // vector<ORB_SLAM3::EdgeSim3ProjectXYZ *> avpEdges12;
    // vector<ORB_SLAM3::EdgeInverseSim3ProjectXYZ *> avpEdges21;
    vector<ORB_SLAM3::EdgeWorldSim3ProjectXYZ *> avpEdges12;
    vector<ORB_SLAM3::EdgeInverseWorldSim3ProjectXYZ *> avpEdges21;
    vector<size_t> avnIndexEdge;
    vector<bool> avbIsInKF2;
    vector<int> avIdsOnlyInKF2;

    int vector_size = 0;
    for (unsigned long edgeKF_i = 0; edgeKF_i < map2KFs.size(); ++edgeKF_i) {
        const std::vector<MapPoint *> &vpMatches1 = avpMatches[edgeKF_i];
        vector_size += vpMatches1.size();
    }
    avnIndexEdge.reserve(2 * vector_size); // TODO 我们这reserve不确定大小，这个只是为了避免内存在push_back时重新分配，所以先不管
    avpEdges12.reserve(2 * vector_size);
    avpEdges21.reserve(2 * vector_size);
    avbIsInKF2.reserve(2 * vector_size);

    for (unsigned long edgeKF_i = 0; edgeKF_i < map2KFs.size(); ++edgeKF_i) {
        // for (int edgeKF_i = 0; edgeKF_i < 1; ++edgeKF_i) {
        KeyFrame *pKF1 = map1KFs[edgeKF_i];
        KeyFrame *pKF2 = map2KFs[edgeKF_i];
        const std::vector<MapPoint *> &vpMatches1 = avpMatches[edgeKF_i];

        g2o::Sim3 gSc1w(pKF1->GetRotation().cast<double>(), pKF1->GetTranslation().cast<double>(), 1.0); // g2o::Sim3: kf 1 world -> kf 1 camera
        g2o::Sim3 gSc2w(pKF2->GetRotation().cast<double>(), pKF2->GetTranslation().cast<double>(), 1.0); // g2o::Sim3: kf 2 world -> kf 2 camera
        ...
        const int N = vpMatches1.size();
        const vector<MapPoint *> &vpMapPoints1 = pKF1->GetMapPointMatches();
        assert(N == vpMapPoints1.size());

        for (int i = 0; i < N; i++) {
            // update record variables
            index++;

            if (!vpMapPoints1[i] || !vpMatches1[i]) // 原来的不考虑vpMapPoints1[i]因为不可能存在无mapPoint而匹配的情况，但现在是2D-2D的Pixel匹配，可能存在这个情况，所以需要加上
                continue;

            MapPoint *pMP1 = vpMapPoints1[i];
            MapPoint *pMP2 = vpMatches1[i];

            const int id1 = 2 * index + 1;
            const int id2 = 2 * (index + 1);

            // @note 之前觉得有bug的地方
            // auto tmp_mvp = pKF2->GetMapPointMatches();
            // const int i2 = std::find(tmp_mvp.begin(), tmp_mvp.end(), pMP2) - tmp_mvp.begin();
            const int i2 = get<0>(pMP2->GetIndexInKeyFrame(pKF2));
            ...
            if (pMP1 && pMP2) {
                ...
                if (!pMP2->isBad()) {
                    ...
                    avIdsOnlyInKF2.push_back(id2);
                }
                continue;
            }

            if (i2 < 0 && !bAllPoints) {
                cout << "    Remove point -> i2: " + to_string(i2) + "; bAllPoints: " + to_string(bAllPoints) << endl;
                continue;
            }

            if (P3D2c(2) < 0) {
                cout << "Sim3: Z coordinate is negative" << endl;
                continue;
            }

            // cerr << "optimizer vertices num: " << optimizer.vertices().size() << endl;
            nCorrespondences++; // 正常的MapPoint匹配对数量
            ...
            if (!pMP2->isEdge) {
                ORB_SLAM3::EdgeWorldSim3ProjectXYZ *e12 = new ORB_SLAM3::EdgeWorldSim3ProjectXYZ(gSc1w, gSc2w);
                ...
                avpEdges12.push_back(e12);
            } else {
                avpEdges12.push_back(NULL);
            }
            ...
            if (!pMP1->isEdge) {
                ORB_SLAM3::EdgeInverseWorldSim3ProjectXYZ *e21 = new ORB_SLAM3::EdgeInverseWorldSim3ProjectXYZ(gSc1w, gSc2w);

                e21->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id1)));
                e21->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
                e21->setMeasurement(obs2);
                const float invSigmaSquare2 = pKF2->mvInvLevelSigma2[kpUn2.octave];
                e21->setInformation(Eigen::Matrix2d::Identity() * invSigmaSquare2);
                ...
                avpEdges21.push_back(e21);
            } else {
                avpEdges21.push_back(NULL);
            }

            avnIndexEdge.push_back(i);
            avbIsInKF2.push_back(inKF2);
        }
    }
    ...
    g2o::VertexSim3Expmap *vSim3_recov = static_cast<g2o::VertexSim3Expmap *>(optimizer.vertex(0));
    gSw1w2 = vSim3_recov->estimate();
    // cout << "测试：Optimize Sim3的尺度变换值，进一步优化：" << gSw1w2.scale() << endl;

    // Check inliers
    int nBad = 0;
    int nBadOutKF2 = 0;
    for (size_t i = 0; i < avpEdges12.size(); i++) {
        ORB_SLAM3::EdgeWorldSim3ProjectXYZ *e12 = avpEdges12[i];
        ORB_SLAM3::EdgeInverseWorldSim3ProjectXYZ *e21 = avpEdges21[i];

        if ((e12 && e12->chi2() > th2) || (e21 && e21->chi2() > th2)) // th2 = 10
        {
            size_t idx = avnIndexEdge[i];
            // vpMatches1[idx] = static_cast<MapPoint *>(NULL); // TODO 应该是保险措施，我们这里取这个有些麻烦，暂时不管
            if (e12)
                optimizer.removeEdge(e12);
            if (e21)
                optimizer.removeEdge(e21);
            avpEdges12[i] = static_cast<ORB_SLAM3::EdgeWorldSim3ProjectXYZ *>(NULL);
            avpEdges21[i] = static_cast<ORB_SLAM3::EdgeInverseWorldSim3ProjectXYZ *>(NULL);
            nBad++;
    ...
    // Recover optimized Sim3
    vSim3_recov = static_cast<g2o::VertexSim3Expmap *>(optimizer.vertex(0));
    gSw1w2 = vSim3_recov->estimate();
    // cout << "测试：Optimize Sim3的尺度变换值，进一步优化：" << gSw1w2.scale() << endl;

    return (float)nIn / (float)avpEdges12.size();
    // return true;
}

float Optimizer::OptimizeCloudPointSim3(const std::vector<KeyFrame *> &cloudKFs, const std::vector<KeyFrame *> &edgeKFs, std::vector<std::vector<MapPoint *>> &avpMatches,
                                        g2o::Sim3 &g2oS12, const float th2, const bool bFixScale, Eigen::Matrix<double, 7, 7> &mAcumHessian, const bool bAllPoints) {
    g2o::SparseOptimizer optimizer;
    ...
    vSim3->setId(0);
    vSim3->setFixed(false);
    vSim3->pCamera1 = cloudKFs[0]->mpCamera; // 赋值了camera_1的成员变量  // TODO CloudMap构建时注意要添加上这个变量
    vSim3->pCamera2 = edgeKFs[0]->mpCamera;  // 赋值了camera_2的成员变量   // TODO CloudMap构建时注意要添加上这个变量
    optimizer.addVertex(vSim3);
    ...
    // result variables
    vector<ORB_SLAM3::EdgeSim3XYZ *> avpEdges12;
    vector<ORB_SLAM3::EdgeInverseSim3XYZ *> avpEdges21;
    vector<size_t> avnIndexEdge;
    vector<bool> avbIsInKF2;
    vector<int> avIdsOnlyInKF2;

    //    avnIndexEdge.reserve(2*N);  // TODO 我们这reserve不确定大小，这个只是为了避免内存在push_back时重新分配，所以先不管
    //    avpEdges12.reserve(2*N);
    //    avpEdges21.reserve(2*N);
    //    avbIsInKF2.reserve(2*N);

    for (int edgeFrontKF_i = 0; edgeFrontKF_i < edgeKFs.size(); ++edgeFrontKF_i) {
        // for (int edgeFrontKF_i = 0; edgeFrontKF_i < 1; ++edgeFrontKF_i) {
        KeyFrame *pKF1 = cloudKFs[edgeFrontKF_i];
        KeyFrame *pKF2 = edgeKFs[edgeFrontKF_i];
        const std::vector<MapPoint *> vpMatches1 = avpMatches[edgeFrontKF_i];
        ...
        for (int i = 0; i < N; i++) {
            ...
            ORB_SLAM3::EdgeSim3XYZ *e12 = new ORB_SLAM3::EdgeSim3XYZ();
            e12->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id2)));
            e12->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
            e12->setMeasurement(Eigen::Vector3d(P3D1c(0), P3D1c(1), P3D1c(2)));
            const float &invSigmaSquare1 = pKF1->mvInvLevelSigma2[kpUn1.octave];
            e12->setInformation(Eigen::Matrix3d::Identity() * invSigmaSquare1);
            g2o::RobustKernelHuber *rk1 = new g2o::RobustKernelHuber;
            rk1->setDelta(deltaHuber);
            e12->setRobustKernel(rk1);
            optimizer.addEdge(e12);
            ...
            ORB_SLAM3::EdgeInverseSim3XYZ *e21 = new ORB_SLAM3::EdgeInverseSim3XYZ();
            e21->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id1)));
            e21->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
            e21->setMeasurement(Eigen::Vector3d(P3D2c(0), P3D2c(1), P3D2c(2)));
            float invSigmaSquare2 = pKF2->mvInvLevelSigma2[kpUn2.octave];
            e21->setInformation(Eigen::Matrix3d::Identity() * invSigmaSquare2);
            g2o::RobustKernelHuber *rk2 = new g2o::RobustKernelHuber;
            rk2->setDelta(deltaHuber);
            e21->setRobustKernel(rk2);
            optimizer.addEdge(e21);
    ...
    g2o::VertexSim3Expmap *vSim3_recov = static_cast<g2o::VertexSim3Expmap *>(optimizer.vertex(0));
    g2oS12 = vSim3_recov->estimate();
    cout << "测试：Optimize Sim3的尺度变换值，进一步优化：" << g2oS12.scale() << endl;

    // Check inliers
    int nBad = 0;
    int nBadOutKF2 = 0;
    for (size_t i = 0; i < avpEdges12.size(); i++) {
        ORB_SLAM3::EdgeSim3XYZ *e12 = avpEdges12[i];
        ORB_SLAM3::EdgeInverseSim3XYZ *e21 = avpEdges21[i];
        if (!e12 || !e21)
            continue;

        // if(e12->chi2()>th2 || e21->chi2()>th2)  // th2 = 10
        if (e12->chi2() > 0.1 || e21->chi2() > 0.1) // th2 = 10
        {
            size_t idx = avnIndexEdge[i];
            //            vpMatches1[idx]=static_cast<MapPoint*>(NULL);  // TODO 应该是保险措施，我们这里取这个有些麻烦，暂时不管
            optimizer.removeEdge(e12);
            optimizer.removeEdge(e21);
            avpEdges12[i] = static_cast<ORB_SLAM3::EdgeSim3XYZ *>(NULL);
            avpEdges21[i] = static_cast<ORB_SLAM3::EdgeInverseSim3XYZ *>(NULL);
            nBad++;
    ...
    // return nIn / (float)avpEdges12.size();
    return true;
}
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF, vector<KeyFrame *> vpFixedKF, bool *pbStopFlag) {
    ...
    // Set fixed KeyFrame vertices
    // @note 往图中添加Fixed Vertices
    int numInsertedPoints = 0;
    for (KeyFrame *pKFi : vpFixedKF) {
        if (pKFi->isBad() || pKFi->GetMap() != pCurrentMap) {
            std::cerr << "ERROR LBA: Fixed KF is bad or is not in the current map" << std::endl;
            continue;
        }
        ...
        // 只添加已经迁移完成的Welding Window的Map Points，这里有一些没在的就不添加
    ...
    // @note 往图中添加non Fixed Vertices
    set<KeyFrame *> spAdjustKF(vpAdjustKF.begin(), vpAdjustKF.end());
    numInsertedPoints = 0;
    for (KeyFrame *pKFi : vpAdjustKF) {
        if (pKFi->isBad() || pKFi->GetMap() != pCurrentMap) {
            std::cerr << "ERROR LBA: Non Fixed KF is bad or is not in the current map" << std::endl;
            continue;
        }
    ...
    // @note 上面主要得到了spKeyFrameBA和vpMPs这两个结果列表

    // 理论上建立的图将有下面这么多数量的Edges
    ...
    if (bDoMore) {
        ...
        Verbose::PrintMess("[BA]: First optimization(Huber), there are " + to_string(badMonoMP) + " monocular and " + to_string(badStereoMP) + " stereo bad edges", Verbose::VERBOSITY_NORMAL);

        // 删除bad edge后再次Optimize
        optimizer.initializeOptimization(0);
        optimizer.optimize(10);
    }
    ...
    // Get Map Mutex
    // @note 开始对Map进行操作，删除坏的的Observation
