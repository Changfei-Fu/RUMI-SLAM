...
Map::Map() :
    ...
    mbFail(false), mIsInUse(false), mHasTumbnail(false), mbHaveMerge(false), mbBad(false), mnMapChangeNotified(0), mbIsInertial(false), mbIMU_BA1(false), mbIMU_BA2(false) {
    mnId = nNextId++;
...
Map::Map(int initKFid) :
    ...
    mnMapChange(0), mbFail(false), mnMapChangeNotified(0), mbHaveMerge(false), mbIsInertial(false), mbIMU_BA1(false), mbIMU_BA2(false) {
    mnId = nNextId++;
    mThumbnail = static_cast<GLubyte *>(NULL);
}

Map::Map(long unsigned int specifyMapId, bool nouse) :
    mnMaxKFid(0), mnBigChangeIdx(0), mbImuInitialized(false), mnMapChange(0), mpFirstRegionKF(static_cast<KeyFrame *>(NULL)),
    mbFail(false), mIsInUse(false), mHasTumbnail(false), mbHaveMerge(false), mbBad(false), mnMapChangeNotified(0), mbIsInertial(false), mbIMU_BA1(false), mbIMU_BA2(false) {
    mnId = specifyMapId;
    mThumbnail = static_cast<GLubyte *>(NULL);
...
vector<KeyFrame *> Map::GetSortedKeyFrames() {
    unique_lock<mutex> lock(mMutexMap);
    auto vKFs = vector<KeyFrame *>(mspKeyFrames.begin(), mspKeyFrames.end());
    struct compFunctor {
        inline bool operator()(KeyFrame *elem1, KeyFrame *elem2) {
            return elem1->mTimeStamp < elem2->mTimeStamp;
        }
    };
...
double Map::KeyFramesDuration() {
    ...
    if (!mspKeyFrames.empty()) {
        ...
        vector<KeyFrame *> vKeyframes(mspKeyFrames.begin(), mspKeyFrames.end());
        sort(vKeyframes.begin(), vKeyframes.end(), compFunctor());
        return vKeyframes[vKeyframes.size() - 1]->mTimeStamp - vKeyframes[0]->mTimeStamp;
    } else {
        return 0;
    }
}

double Map::KeyFramesLength() {
    unique_lock<mutex> lock(mMutexMap);
    if (!mspKeyFrames.empty()) {
        return ComputeTrajLength();
    } else {
        return 0;
    }
}
...
void Map::clear() {
    ...
    for (set<KeyFrame *>::iterator sit = mspKeyFrames.begin(), send = mspKeyFrames.end(); sit != send; sit++) {
        KeyFrame *pKF = *sit;
        // @note 如果不将KF从KeyframeDatabase中清除，LoopClosure中会可能用上它
        pKF->GetKeyFrameDatabase()->erase(pKF);
...
void Map::SetHaveMerged() {
    mbHaveMerge = true;
}

void Map::ResetHaveMerged() {
    mbHaveMerge = false;
}

bool Map::IsHaveMerged() {
    return mbHaveMerge;
}

void Map::SortKF(vector<KeyFrame *> &vpKFs) {
    struct compFunctor {
        inline bool operator()(KeyFrame *elem1, KeyFrame *elem2) {
            return elem1->mTimeStamp < elem2->mTimeStamp;
        }
    };
    sort(vpKFs.begin(), vpKFs.end(), compFunctor());
}

float Map::ComputeKFTrajLength(vector<KeyFrame *> vpKFs) {
    SortKF(vpKFs);
    int nKFNum = vpKFs.size();
    float trajLength = 0;
    for (int i = 1; i < nKFNum; i++) {
        auto prevKFCenter = vpKFs[i - 1]->GetCameraCenter();
        auto curKFCenter = vpKFs[i]->GetCameraCenter();
        trajLength += sqrt(pow(prevKFCenter.x() - curKFCenter.x(), 2) + pow(prevKFCenter.y() - curKFCenter.y(), 2) + pow(prevKFCenter.z() - curKFCenter.z(), 2));
    }
    return trajLength;
}

float Map::ComputeKFCurvature(vector<KeyFrame *> vpKFs) {
    SortKF(vpKFs);
    float trajLength = ComputeKFTrajLength(vpKFs);
    auto prevKFCenter = vpKFs[0]->GetCameraCenter();
    auto curKFCenter = vpKFs[vpKFs.size() - 1]->GetCameraCenter();
    float directLength = sqrt(pow(prevKFCenter.x() - curKFCenter.x(), 2) + pow(prevKFCenter.y() - curKFCenter.y(), 2) + pow(prevKFCenter.z() - curKFCenter.z(), 2));
    return trajLength / directLength;
}

float Map::ComputeTrajLength() {
    vector<KeyFrame *> vpKFs = GetAllKeyFrames();
    SortKF(vpKFs);
    return ComputeKFTrajLength(vpKFs);
}

float Map::ComputeTrajCurvature() {
    vector<KeyFrame *> vpKFs = GetAllKeyFrames();
    SortKF(vpKFs);
    return ComputeKFCurvature(vpKFs);
}
